...
cpdef np.ndarray uniform_temperature(list boreholes, time, double alpha, self, int nSegments=12, method='linear',
                                     bint use_similarities=True, double disTol=0.01, double tol=1.0e-6,
                                     processes=None, bint disp=True):
    """
...
cpdef np.ndarray load_history_reconstruction(np.ndarray time, np.ndarray Q):
    """
...
cpdef list _borehole_segments(list boreholes, int nSegments):
    """
    Split boreholes into segments.
...
cpdef np.ndarray _temporal_superposition(np.ndarray dh_ij, np.ndarray Q):
    """
